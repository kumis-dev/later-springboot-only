...
public class ItemNoteServiceImpl implements ItemNoteService {
    ...
    @Override
    @Transactional
    public ItemNoteDto addNewItemNote(long userId, ItemNoteDto dto) {
        Item item = itemRepository.findById(dto.getItemId())
                .orElseThrow(() -> new RuntimeException("Item not found"));
        ItemNote itemNote = itemNoteRepository.save(ItemNoteMapper.mapToItemNote(dto, item));
    ...
    public List<ItemNoteDto> searchNotesByTag(long userId, String tag) {
        List<ItemNote> itemNotes = itemNoteRepository.findItemNoteByUserIdAndTag(userId, tag);
        return ItemNoteMapper.mapToItemNoteDto(itemNotes);
    }
